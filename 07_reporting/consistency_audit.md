...
| Fig. 4 viability summary | `lambda = 0.967`, bootstrap interval `0.828-1.059`, `P(decline) = 86.0%`, LOSO `0.882-0.993` | `06_analysis/output/population_parameters.csv` | match | Fig. 4 text now reflects the current transition-matrix outputs. |
| Fig. 4 elasticity interpretation | SC5 stasis contributes `57.4%` of matrix-cell elasticity; aggregated SC5 survival elasticity is `58.8%` | `06_analysis/output/elasticity_matrix.csv`, `06_analysis/output/vital_rate_elasticity.csv` | match | The legend now distinguishes matrix-cell and aggregated survival elasticity correctly. |
