...
| PubMed/Semantic Scholar hit counts (March 2026 expansion) | Lines 122-123 (§3.2 table) | Number of hits from the expanded search. | These searches were run in a separate session; not in Raine's notes. Recoverable from session logs. |
| Exact search strings for March 2026 expansion | Line 128 (§3.2) | The PubMed and Semantic Scholar query strings used during the expanded search. | Run during expanded search, not by Raine. Recoverable from session logs. |
