...
| Verify k counts (k=16 studies, 21 effects) | Line 345 (§6.1 note) | Cross-check the final study count against the actual output of `14b_expanded_meta_analysis.R`. Roth et al. 2013 removed for data overlap with Rogers & Muller 2012. NOAA split by region (FL Keys/Curacao/Navassa). Garrison & Ward 2008 split into control/relocated (2 effects). | This is a code verification task, not in the notes. Requires running or inspecting the R script output. |
