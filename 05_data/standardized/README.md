...
**Biological-realism scenario framework inputs (scripts 50–61):**

- `apal_outplant_age.csv` — `years_since_outplant` per coral_id for restoration studies (generated by `50_derive_outplant_age.R`; consumed by `56_outplant_age_model.R`).
- `apal_lesion_state.csv` — per-colony lesion flag from tissue-loss intervals >10% (generated by `52_colony_lesion_state.R`; consumed by `55_lesion_fecundity_penalty.R`).
- `apal_lesion_population_fraction.csv` — per-(study × size class) fraction-lesioned summary for reproductive adults.
- `winter_sst_anomalies.csv` — region × year winter (Jan–Mar) SST and anomalies vs 1982–2010 climatology, from NOAA ERSST v5 via ERDDAP (generated by `51_winter_sst_anomalies.R`; consumed by `57_winter_sst_survival_model.R`). Cached monthly data live in `05_data/external/noaa_oi_sst/region_*_monthly.rds`.
