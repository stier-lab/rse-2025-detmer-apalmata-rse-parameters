...
#
# OUTPUT:
#   analysis/figures/supplementary/FigS3_model_diagnostics.{png,pdf}
#   analysis/figures/supplementary/FigS4_model_selection.{png,pdf}
...
surv_data <- readRDS(file.path(project_root, "analysis/output/prepared_survival_data.rds")) %>%
  filter(!is.na(size_class), !is.na(survived), !is.na(log_size))

growth_data <- readRDS(file.path(project_root, "analysis/output/prepared_growth_data.rds")) %>%
...
surv_models <- read.csv(file.path(project_root, "analysis/output/model_selection_survival.csv"),
                        stringsAsFactors = FALSE)
growth_models <- read.csv(file.path(project_root, "analysis/output/model_selection_growth.csv"),
                          stringsAsFactors = FALSE)
